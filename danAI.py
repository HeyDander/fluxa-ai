...
LOCAL_KNOWLEDGE = {
    ...
    "машинн обуч": "Машинное обучение — это подход, где модель учится на примерах вместо того, чтобы получать все правила вручную.",
    "transformer": "Transformer — это архитектура нейросети, которая хорошо работает с последовательностями и контекстом. На ней построено много современных языковых моделей.",
    "токен": "Токен — это кусочек текста, с которым работает модель. Это может быть слово, часть слова или знак.",
    ...
    "привычк": "Привычку проще всего строить на маленьком действии, которое повторяется в одно и то же время и не требует героизма.",
    "учеб": "Учёба идёт лучше, когда теория сразу закрепляется практикой: короткий блок, один пример, одна самостоятельная попытка.",
    "экзамен": "Подготовка к экзамену лучше работает по блокам: темы, ключевые вопросы, короткое повторение и тренировка на задачах.",
    ...
    "sql": "SQL — это язык запросов к базе данных. Им получают, добавляют, изменяют и удаляют данные в таблицах.",
    ...
    "fastapi": "FastAPI — это Python-фреймворк для API. Его любят за скорость, типизацию и удобную автоматическую документацию.",
    "flask": "Flask — это лёгкий Python-фреймворк для веба. Он хорошо подходит для небольших сервисов и прототипов.",
    "django": "Django — это крупный Python-фреймворк с готовой админкой, ORM, маршрутизацией и системой пользователей.",
    "telegram": "Telegram-бот обычно строится вокруг команд, обработчиков сообщений, кнопок и логики хранения состояния пользователя.",
    "сайт": "Сайт обычно состоит из интерфейса, логики, хранения данных и маршрутов. Хороший старт — сначала сделать минимальный рабочий сценарий пользователя.",
    "чат": "Чат обычно требует историю сообщений, отправку, обновление в реальном времени, хранение пользователей и ограничения против спама.",
    "авторизац": "Авторизация нужна, чтобы система понимала, кто именно вошёл в аккаунт. Чаще всего это сессии, куки или токены.",
    "регистрац": "Регистрация обычно включает логин, пароль, валидацию, хранение пользователя и потом вход через сессию или токен.",
...
class SmartChatBot:
    ...
    def _feature_request_reply(self, message: str) -> str | None:
        ...
        if "профил" in normalized:
            return self._add_emoji(
                "Профиль лучше делать с минимального набора: имя, аватар, короткое описание, настройки и история действий. Потом уже можно добавлять статусы, достижения и оформление.",
                message,
            )

        if "уведомлен" in normalized:
            return self._add_emoji(
                "Уведомления лучше строить так: кнопка включения, хранение подписки, понятные системные сообщения и отдельные типы событий, чтобы можно было не спамить пользователя.",
                message,
            )

        if "авторизац" in normalized or "логин" in normalized or "регистрац" in normalized:
            return self._add_emoji(
                "Нормальная авторизация делается так: регистрация, проверка логина и пароля, хэширование пароля, сессия или токен и защита от выхода после обновления страницы.",
                message,
            )
    ...
    def _local_knowledge_answer(self, message: str) -> str | None:
        ...
        for marker, explanation in LOCAL_KNOWLEDGE.items():
            if marker in normalized:
                ...
                if question_type == "request":
                    return self._add_emoji(f"{explanation} Если делать это в проекте, начни с минимальной рабочей версии и не усложняй первый шаг.", message)
                return self._add_emoji(explanation, message)
        return None

    def _local_general_answer(self, message: str, matches: list[tuple[float, dict]]) -> str:
        question_type = classify_question(message)
